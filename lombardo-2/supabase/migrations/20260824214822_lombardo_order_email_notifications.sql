-- Add email as a server-only operational notification channel while preserving
-- the existing WhatsApp outbox and RPC for deployments already in flight.

alter table public.commerce_order_notifications
  drop constraint commerce_order_notifications_channel_check;

alter table public.commerce_order_notifications
  add constraint commerce_order_notifications_channel_check check (
    channel in ('whatsapp_cloud_api', 'email_resend')
  );

create or replace function public.lombardo_claim_order_notification_v2(
  p_tenant_id text,
  p_order_id bigint,
  p_channel text,
...
revoke all on function public.lombardo_claim_order_notification_v2(text, bigint, text, boolean)
  from public, anon, authenticated;
grant execute on function public.lombardo_claim_order_notification_v2(text, bigint, text, boolean)
  to service_role;

comment on function public.lombardo_claim_order_notification_v2(text, bigint, text, boolean) is
  'Claims one idempotent operational notification per order and channel.';

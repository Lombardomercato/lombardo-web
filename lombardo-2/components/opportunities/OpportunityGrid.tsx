...
export function OpportunityGrid({ products, surface, recommendedProductId }: {
  products: Product[];
  ...
  return (
    <div className={styles.grid} data-surface={surface}>
      {products.map((product, index) => {
        ...
        const recommended = product.id === recommendedProductId;
        return (
          <article
            className={styles.card}
            data-index={String(index + 1).padStart(2, "0")}
            data-kind={recommended ? "recommendation" : undefined}
            key={product.id}
          >
            <Link
            ...
            <div className={styles.details}>
              <p>
                {recommended
                  ? `RECOMENDACIÓN LOMBARDO · ${product.brand.name}`
